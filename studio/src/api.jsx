// ---------------------------------------------------------------------------
// Deadman Studio · API seam.
//
// This is the ONLY place that talks to data. Today it returns mock objects
// from data.jsx after a small delay. To wire the real producer graph, replace
// each body with a fetch() to the corresponding backend route — the UI never
// changes because the returned shapes match the on-disk artifacts.
//
// Suggested real routes (producer-only, behind operator auth):
//   POST /api/studio/runs                       -> create + start a run
//        (server runs: deadman_run_producer_graph.py start --run-id … )
//        body: { drama_id, drama_title, graph_mode, episodes[] }  (MP4 upload)
//   GET  /api/studio/runs                        -> list run manifests
//   GET  /api/studio/runs/{run_id}               -> one manifest (poll while running)
//   GET  /api/studio/runs/{run_id}/report        -> final_report.md (text)
//   GET  /api/studio/runs/{run_id}/review        -> review_request.json + shortlist
//   POST /api/studio/runs/{run_id}/resume        -> { decision, kept_ids[], note }
//        (server runs: deadman_run_producer_graph.py resume --review-decision … )
// ---------------------------------------------------------------------------
...
// In-memory store standing in for server state, so created runs + the resume
// flow persist within the session (mirrors manifest + checkpoint behavior).
const store = {
  runs: clone(RUNS),
...
// Build a review shortlist for a freshly created run by mapping the demo
// candidate bodies onto the uploaded episodes.
...
// Ordered ingest+mining nodes that animate before the gate (mode-aware).
...
const StudioAPI = {
  async listRuns() { await wait(80); return clone(store.runs); },
  ...
  async startRun(payload) {
    ...
    const run = {
      run_id: runId, thread_id: runId,
      drama_id: payload.dramaId, drama_title: payload.dramaTitle,
      graph_mode: payload.mode, status: "running",
      started_at: nowStr(), updated_at: nowStr(),
      current_node: "prepare_assets", recall_budget: recall,
  ...
  publishSequence(runId) {
    ...
    const seq = ["llm_drama_context_draft", "llm_moment_pack_draft",
